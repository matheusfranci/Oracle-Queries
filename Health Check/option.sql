...
ttitle center 'OVERALL INFORMATION' skip 2
select i.HOST_NAME,
       i.INSTANCE_NAME,
...
ttitle center 'MULTITENANT INFORMATION (Please ignore errors in pre 12.1 databases)' skip 2
...
ttitle center 'DBA_FEATURE_USAGE_STATISTICS (DBA_FUS) INFORMATION - MOST RECENT SAMPLE BASED ON LAST_SAMPLE_DATE' skip 2
select distinct
       &&DCID as CON_ID,
...
ttitle center 'PRODUCT USAGE' skip 2
with
MAP as (
...
ttitle center 'FEATURE USAGE DETAILS' skip 2
with
MAP as (
-- mapping between features tracked by DBA_FUS and their corresponding database products (options or packs)
select '' PRODUCT, '' feature, '' MVERSION, '' CONDITION from dual union all
