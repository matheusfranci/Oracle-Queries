## Explicação do Procedimento `coe_xfr_sql_profile.sql`

Este script SQL*Plus tem como objetivo gerar um outro script SQL (`coe_xfr_sql_profile_<sql_id>_<plan_hash_value>.sql`) que contém os comandos necessários para criar manualmente um SQL Profile customizado no Oracle Database. Este SQL Profile é baseado em um plano de execução conhecido, que pode ser obtido da memória (SGA) ou do Automatic Workload Repository (AWR).

O script gerado pode ser executado no mesmo sistema de origem ou em um sistema de destino similar que possua os mesmos objetos de esquema referenciados pela consulta SQL.

**Pré-requisitos:**

* Licença do Oracle Tuning Pack (necessário para usar `DBMS_SQLTUNE`).
* Acesso ao banco de dados Oracle como `SYSDBA` ou um usuário com privilégios para acessar o dicionário de dados.

**Parâmetros de Entrada:**

O script recebe dois parâmetros, solicitados interativamente ou passados na linha de comando:

1.  **`SQL_ID` (obrigatório):** O identificador único da consulta SQL para a qual se deseja criar o SQL Profile.
2.  **`PLAN_HASH_VALUE` (obrigatório):** O valor hash do plano de execução específico que será usado como base para o SQL Profile. Uma lista de planos conhecidos para o `SQL_ID` fornecido é apresentada para auxiliar na escolha.

**Fluxo do Procedimento:**

1.  **Configuração do Ambiente SQL*Plus:**
    * Define um arquivo de log (`coe_xfr_sql_profile.log`).
    * Configura diversas opções do SQL*Plus para controlar a saída (desativa eco de comandos, feedback, cabeçalhos, etc.).

2.  **Apresentação dos Parâmetros:**
    * Solicita e define os valores para `sql_id` e `plan_hash_value` através da diretiva `DEF`.
    * Exibe os valores fornecidos para confirmação.

3.  **Consulta de Planos Conhecidos:**
    * Executa uma consulta SQL para listar os `plan_hash_value` associados ao `sql_id` fornecido, tanto da memória (`gv$sql_plan`) quanto do AWR (`dba_hist_sql_plan`).
    * Tenta obter o tempo médio de execução para cada plano (`avg_et_secs`) para ajudar o usuário a identificar o plano desejado.

4.  **Recuperação do Texto SQL:**
    * Declara uma variável do tipo `CLOB` (`:sql_text`) para armazenar o texto SQL.
    * Primeiro, tenta obter o texto SQL da memória (`gv$sqltext_with_newlines`).
    * Se não encontrar na memória, tenta recuperá-lo do AWR (`dba_hist_sqltext`).
    * Exibe o texto SQL recuperado para o usuário.

5.  **Recuperação do `other_xml` (Informações do Plano):**
    * Declara uma variável do tipo `CLOB` (`:other_xml`) para armazenar os detalhes do plano de execução em formato XML.
    * Primeiro, tenta obter o `other_xml` da memória (`gv$sql_plan`) para o `sql_id` e `plan_hash_value` especificados.
    * Se não encontrar na memória, tenta recuperá-lo do AWR (`dba_hist_sql_plan`).
    * Exibe o `other_xml` recuperado.

6.  **Geração do Script de Criação do SQL Profile:**
    * Desativa o `ECHO` para a saída principal.
    * Define o nome do arquivo de saída do script gerado (`coe_xfr_sql_profile_<sql_id>_<plan_hash_value>.sql`).
    * Ativa o `SPOOL` para direcionar a saída para o arquivo.
    * Dentro de um bloco PL/SQL:
        * Imprime comandos SQL*Plus para configurar o ambiente do script gerado (log, echo, etc.).
        * Adiciona comentários com informações sobre o script gerado (autor, descrição, parâmetros, exemplo de execução, notas).
        * Declara uma variável para armazenar a assinatura SQL (`:signature`).
        * Inicia um bloco `DECLARE` para construir o script de criação do SQL Profile:
            * Define uma variável `sql_txt` do tipo `CLOB` e atribui a ela o texto SQL recuperado, utilizando a sintaxe de string literal (`q'[]'`).
            * Define uma variável `h` do tipo `SYS.SQLPROF_ATTR` para armazenar os atributos do SQL Profile (as hints do plano de execução).
            * Extrai as hints do `other_xml` utilizando funções XML (`XMLTYPE`, `XMLSEQUENCE`, `EXTRACTVALUE`).
            * Formata as hints e as adiciona à variável `h`.
            * Calcula a assinatura SQL usando `DBMS_SQLTUNE.SQLTEXT_TO_SIGNATURE`.
            * Gera a chamada para o procedimento `DBMS_SQLTUNE.IMPORT_SQL_PROFILE` para criar o SQL Profile:
                * `sql_text`: O texto SQL.
                * `profile`: Os atributos do perfil (`h`).
                * `name`: O nome do SQL Profile (`prof_<sql_id>_<plan_hash_value>`).
                * `description`: Uma descrição do perfil.
                * `category`: A categoria do perfil (`DEFAULT`).
                * `validate`: Indica se o perfil deve ser validado (`TRUE`).
                * `replace`: Indica se um perfil existente com o mesmo nome pode ser substituído (`TRUE`).
                * `force_match`: Controla como o SQL Profile é correspondido a novas execuções SQL (`TRUE` para correspondência mesmo com literais diferentes, `FALSE` para correspondência exata).
            * Finaliza o bloco PL/SQL.
        * Imprime comandos para exibir a assinatura SQL e mensagens informativas.
    * Desativa o `SPOOL`.

7.  **Finalização:**
    * Restaura as configurações originais do SQL*Plus.
    * Exibe mensagens informando o nome do script gerado e a instrução para executá-lo no sistema de destino.
    * Remove as definições das variáveis `sql_id` e `plan_hash_value`.
    * Exibe uma mensagem de conclusão.

Em resumo, o script `coe_xfr_sql_profile.sql` automatiza o processo de extração de um plano de execução específico para uma determinada consulta SQL e gera um script que pode ser usado para criar um SQL Profile customizado baseado nesse plano, permitindo influenciar o otimizador do Oracle a utilizar o plano desejado.
...
SPO OFF;
SPO c:\temp\coe_xfr_sql_profile_&&sql_id._&&plan_hash_value..sql;
DECLARE
  l_pos NUMBER;
  l_hint VARCHAR2(32767);
BEGIN
  DBMS_OUTPUT.PUT_LINE('SPO c:\temp\coe_xfr_sql_profile_&&sql_id._&&plan_hash_value..log;');
  ...
  DBMS_OUTPUT.PUT_LINE('name        => ''prof_&&sql_id._&&plan_hash_value.'',');
  DBMS_OUTPUT.PUT_LINE('description => ''profile &&sql_id. &&plan_hash_value. ''||:signature||'''',');
...
PRO
PRO Execute c:\temp\coe_xfr_sql_profile_&&sql_id._&&plan_hash_value..sql
